...
CUBOS_REFLECT_IMPL(roll_racers::BlackoutDestroy)
{
    return cubos::core::ecs::TypeBuilder<BlackoutDestroy>("roll_racers::BlackoutDestroy").build();
}

CUBOS_REFLECT_IMPL(roll_racers::Semaphore)
{
    return cubos::core::ecs::TypeBuilder<Semaphore>("roll_racers::Semaphore")
        .withField("stage", &Semaphore::stage)
        .withField("intensity", &Semaphore::intensity)
        .build();
}

void roll_racers::blackoutPlugin(Cubos& cubos)
{
